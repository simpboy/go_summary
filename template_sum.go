...
func (p Person_sum) Say(msg string) string {
	return msg;
}
...
func main() {
	...
	t3.Execute(os.Stdout, Person_sum{"001", "test", 11, nil});
	fmt.Println();
	...
	t4.Execute(os.Stdout, Person_sum{"002", "test2", 22, nil});
	fmt.Println();
	...
	t6, _ = t6.Parse(`
                      ...
                      {{.UserName|test2}}
                     `);
	t6.Execute(os.Stdout, Person_sum{"003", "test3", 33, nil});
	fmt.Println();
	...
	con["qq"] = "123456";
	con["tel"] = "13888888888";
	t8.Execute(os.Stdout, Person_sum{Contact: con});
	fmt.Println();
	...
	t10.Execute(os.Stdout, Person_sum{Contact: con2});
}